...
**Provider coverage.** Works for any provider that offers IMAP with app passwords: iCloud, Gmail (with 2FA enabled), Yahoo, Fastmail, AOL. For providers that are OAuth-only via Mail.app's UI (some enterprise Outlook configurations), the user would need to switch to app-password auth or fall back to AppleScript. This is documented, not blocked on.
...
3. **Empty-mailbox test caveat.** The 2026-04-22 iCloud spike authenticated against an Apple ID whose IMAP inbox is empty (user merged two Apple IDs; residual mailbox). FETCH envelope was not exercised. Integration tests on #41's delivery should include FETCH against a populated mailbox.
